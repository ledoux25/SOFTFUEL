...
$scope.prix_unitaire_gaz_ab = function(){

  var prix_unitaire_ab = 0;

   angular.forEach($scope.recap.Gaz_AB, function(value) {

    prix_unitaire_ab = prix_unitaire_ab + parseInt(value.prix_unitaire_ab) ;  
    
  });

  console.log(prix_unitaire_ab);

   return prix_unitaire_ab ;

}

$scope.prix_unitaire_gaz_sb = function(){

  var prix_unitaire_sb = 0;

   angular.forEach($scope.recap.Gaz_SB, function(value) {

    prix_unitaire_sb = prix_unitaire_sb + parseInt(value.prix_unitaire_sb) ;  
    
  });

  console.log(prix_unitaire_sb);

   return prix_unitaire_sb ;
...
$scope.recapGaz_AB = function(){
 $scope.recap.Gaz_AB = [];
 //d
 console.log($scope.app.quart.gaz); 
 angular.forEach($scope.app.quart.gaz, function(value) {
    ...
    var obj = {};
      ...
      $scope.recap.Gaz_AB.push(obj); 
       console.log('recap Gaz_AB 2');
       console.log((obj.montant_ab));
       console.log((obj.qv_ab));
       console.log((obj.prix_unitaire_ab));
    ...
    var obj2 = {nom_produit:'Total Ventes Avec Bouteille', qv_ab:$scope.qte_gaz_ab(), prix_unitaire_ab:$scope.prix_unitaire_gaz_ab(), montant_ab:$scope.mont_gaz_ab()};
...
$scope.recapGaz_SB = function(){
  $scope.recap.Gaz_SB = [];
  console.log('voici app recharge');
  console.log($scope.app.quart.gaz_recharge);
  angular.forEach($scope.app.quart.gaz_recharge, function(value) {
     if (value.qv_sb != 0) {
     var obj = {};
       obj.nom_produit = value.nom_produit;
       obj.code_produit = value.code_produit;
    ...
    console.log(obj);
     ...
     var obj2 = {nom_produit:'Total Ventes par Recharge', qv_sb:$scope.qte_gaz_sb(), prix_unitaire_sb:$scope.prix_unitaire_gaz_sb(), montant_sb:$scope.mont_gaz_sb()};
...
$scope.produit_gaz_rechange =  function(){
  $http.get('api/pompistes.php?action=get_produit_gaz_rechange').success( function(data, status, headers, config){
      $scope.produit_gaz_rechange = data ;
      console.log($scope.produit_gaz_rechange);
...
if ($scope.app.quart.cuves) {
  ...
  $localStorage.Quart1.gaz_recharge = $scope.app.quart.gaz_recharge ;//d
...
console.log($localStorage.Quart1) ;
  ...
  $localStorage.Quart1.gaz_recharge = $scope.produit_gaz_rechange;//d
  ...
  $scope.app.quart.gaz_recharge = $localStorage.Quart1.gaz_recharge;//d
...
// console.log($en_tete.vers_clients) ;
 ...
 /**********************************FIN TRAITEMENT EN-TETE********************************** */
  ...
  angular.forEach($scope.recap.Carburant, function($value2) {
   ...
   angular.forEach($scope.recap.Gaz_AB, function($value3) {

    if ($value3.nom_produit == 'Total Ventes Avec Bouteille') {
                
      $data.total_gaz = $value3.montant_ab ;
   ...
   angular.forEach($scope.recap.Gaz_SB, function($value3) {

    if ($value3.nom_produit == 'Total Ventes par Recharge') {
                
      $data.total_gaz = $data.total_gaz  + $value3.montant_sb ;
  ...
  $data.versement = $scope.app.quart.versement.montant ;
   ...
   $data.total_attendu = $data.total_carb + $data.total_lub + $data.total_gaz ;
...
var obj = {
              ...
              'app_quart_gaz'     :  $scope.app.quart.gaz, 
              'app_quart_gaz_recharge'     :  $scope.app.quart.gaz_recharge,  
...
if ( obj.app_quart_gaz.length > 0) {

  angular.extend(obj2, {'app_quart_gaz'     :  $scope.app.quart.gaz});
  
}

if ( obj.app_quart_gaz_recharge.length > 0) {

  angular.extend(obj2, {'app_quart_gaz_recharge'     :  $scope.app.quart.gaz_recharge});
