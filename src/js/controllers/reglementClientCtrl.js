app.controller('ModalInstanceBeatCtrl', ['$scope', '$modalInstance', 'items', function($scope, $modalInstance, items) {
  ...
  app.controller('reglementClientCtrl', ['$scope', '$filter', '$http', '$localStorage',
  ...
  $http.post('api/pompistes.php?action=addRgt_cli',
              ...
              if (data.msg == "") {
                ...
                alert('Félicitations, opération effectuée avec succes') ;

                $scope.dt.montant = 0 ;
                $scope.dt.libelle_reglement = '' ;
                  
              } else {

                alert('Désolé, Une erreur est produite') ;
  ...
  $scope.getRgt_cli = function(){
  
  console.log($scope.dt);
  
  $http.post('api/pompistes.php?action=getRgt_cli',
